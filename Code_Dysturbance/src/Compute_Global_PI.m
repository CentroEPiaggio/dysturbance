...
Global_PI_folder = strcat('.\tests\', data_folder,'\Global_PIs');
mkdir(Global_PI_folder);

% Now we must retrieve the local PI data from each experiment folder
Experiment_folder = strcat('.\tests\', da7ta_folder);
...
j = 0;
for i = 1:num_dir
	if all_dir(i).name ~= "."  &&  all_dir(i).name ~= ".."
		j = j + 1;
		Tests_folders(j) = all_dir(i);
	end
end
